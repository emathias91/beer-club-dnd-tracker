...
| 5 | **DM-force "Reload sheet" prompt not appearing for the seat holder** | **Could not reproduce (2026-09-05) — needs more specific repro from reporter** |

Residual risk: full `POST /api/state` still exists for import/compat paths; prefer piece APIs in normal play.

**#5 detail:** when the DM edits a character's data (e.g. HP via the damage/heal modal) while a player holds that seat, the player is supposed to get a "DM changed &lt;character&gt;... Reload sheet to see updates" confirm prompt so their local sheet doesn't silently go stale. Originally reported as not firing.
- **2026-09-05 investigation:** ran a real two-session repro (one browser tab claiming the player seat on a character, a second tab as DM editing that character's HP) against the test game. The mechanism worked correctly in both scenarios tried: (a) a pre-existing unacknowledged `lastDmForce` fired the confirm prompt immediately on a fresh player-seat claim/boot; (b) a live DM HP edit while the player tab stayed connected (backgrounded, not focused) was picked up by the next ~3s poll and fired the prompt correctly. Confirmed end-to-end via direct state inspection: `dmForces` populated correctly, `confirm()` fired with the right message, and after acknowledging, the player's local HP updated to match and `dmForces` cleared.
- **Could not reproduce the reported failure.** Possible explanations, not yet ruled out: (a) the player session at the time wasn't actually holding a live seat claim (`processSeatNotices()` in `js/sync.js` silently no-ops if `SeatSession.get()` isn't `role: 'player'` with a matching `characterId` — no error, no log, easy to mistake for "the feature is broken" if the seat claim had quietly lapsed or was on a different device/session than expected); (b) real-browser dialog suppression — Chrome offers "Prevent this page from creating additional dialogs" after repeated `confirm()`/`alert()` calls from the same origin in a session, which would silently swallow this exact prompt if it had been checked earlier in that browser; (c) the user's description ("prompting user to confirm changes") may refer to the separate **offer** flow (`state.offers[charId]`, a different code path for a non-holder player editing a character they don't hold) rather than this DM-force flow — worth confirming which prompt wording was actually seen.
- **Next step if this recurs:** get exact repro steps (which browser/device for each side, whether the player seat was freshly claimed or already held, exact prompt wording if any partial message appeared) rather than re-investigating blind — the mechanism itself is confirmed working as designed.
