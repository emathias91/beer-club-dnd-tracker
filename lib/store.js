...
/** Observability (P3 #18): "who saved last" travels with the document itself,
 *  not just in server logs that rotate away. */
function stampLastSavedBy(doc, label, role) {
    doc.lastSavedBy = { label: label || 'Unknown', role: role || null, at: nowIso() };
}
...
/**
 * Legacy full-state mirror, written on every full-state POST (server.js
 * saveStateSafely()). It is NOT the live source of truth — that's the split
 * per-document layout below (manifest.json + campaign/<id>/{meta,map,combat}.json
 * + campaign/<id>/characters/<charId>.json, all via writeDoc()). The monolith
 * is kept only for backward-compatible GET /api/state and as the one-time
 * migrateIfNeeded() source. Rolling backups (below) snapshot the split layout
...
function writeDoc(filePath, doc) {
    ...
    // Every meta/map/combat/character write funnels through here (piece-saves via
    // putPiece(), full-state writes via writeSplitFromState()) — single hook point
    // for rolling backups, throttled internally.
    try { createBackupSnapshot({ force: false, reason: 'auto' }); } catch (e) { /* best effort */ }
}
...
function getSession(token) {
    ...
    if (!sess) return null;
    // Revoked (e.g. seat stolen): not valid for API writes; heartbeat still sees raw row.
    if (sess.revoked) return null;
    return { token, ...sess };
}

function getSessionRaw(token) {
    if (!token) return null;
    const all = readSessions();
    const sess = all.sessions[token];
    if (!sess) return null;
...
/** Drop a session token without touching character claims (used after seat_taken notify). */
function revokeSessionOnly(token) {
    if (!token) return false;
    const all = readSessions();
    if (!all.sessions[token]) return false;
    delete all.sessions[token];
    writeSessions(all);
    return true;
}

/** Mark session dead for writes immediately; heartbeat delivers seat_taken then deletes. */
function markSessionSeatTaken(token) {
    if (!token) return false;
    const all = readSessions();
    if (!all.sessions[token]) return false;
    all.sessions[token].revoked = 'seat_taken';
    all.sessions[token].revokedAt = nowIso();
    writeSessions(all);
    return true;
}
...
function writeSplitFromState(state, savedBy) {
    ensureDir(dataRoot());
    const campaigns = state.campaigns || [];
    const label = (savedBy && savedBy.label) || 'Unknown';
    const role = (savedBy && savedBy.role) || null;

    // Rolling backups now snapshot the split layout itself (see createBackupSnapshot,
    // hooked into writeDoc()) — this no longer copies the legacy monolith file.
    ...
    for (const camp of campaigns) {
        ...
        stampLastSavedBy(metaDoc, label, role);
        ...
        stampLastSavedBy(mapDoc, label, role);
        ...
        stampLastSavedBy(combatDoc, label, role);
        ...
        for (const [charId, data] of Object.entries(chars)) {
            ...
            stampLastSavedBy(doc, label, role);
...
/* ---------- rolling backups (split-layout snapshot, gzip) ---------- */

// Random suffix guards against two backups (e.g. a manual one immediately
// followed by a pre-restore safety snapshot) landing in the same millisecond
// and silently overwriting each other.
function backupFileName(stamp) {
    return `backup-${stamp}-${crypto.randomBytes(3).toString('hex')}.json.gz`;
}

/**
 * Snapshot the current split-layout files (manifest + every campaign's
 * meta/map/combat/character docs) straight off disk into one gzipped JSON
 * bundle. Deliberately excludes sessions.json (stale seat claims shouldn't be
 * restored) and dm_notes.json (backed up separately, PIN-encrypted — see
 * createDmNotesBackupSnapshot in server.js).
 *
 * Throttled to at most once per AUTO_BACKUP_MIN_INTERVAL_MS unless
 * opts.force is set (manual "Back Up Now", and the safety snapshot taken
 * right before a restore).
 */
function createBackupSnapshot(opts = {}) {
    const force = !!opts.force;
    const reason = opts.reason || (force ? 'manual' : 'auto');
    ...
    if (!force) {
        const existing = fs.readdirSync(dir)
            ...
            .sort();
        if (existing.length) {
            const latest = fs.statSync(path.join(dir, existing[existing.length - 1]));
            if (Date.now() - latest.mtimeMs < AUTO_BACKUP_MIN_INTERVAL_MS) return null;
        }
    }
    ...
    for (const c of manifest.campaigns || []) {
        ...
        for (const charId of listCharacterIds(cid)) {
            characters[charId] = readDoc(characterPath(cid, charId));
        }
        campaigns[cid] = {
            meta: readDoc(metaPath(cid)),
            map: readDoc(mapPath(cid)),
            combat: readDoc(combatPath(cid)),
            characters
        };
    }

    const bundle = { createdAt: nowIso(), reason, manifest, campaigns };
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    ...
    while (all.length > MAX_BACKUPS) {
        try { fs.unlinkSync(path.join(dir, all.shift())); } catch (e) { /* best effort */ }
    }

    return { fileName, createdAt: bundle.createdAt };
}

function readBackupBundle(fileName) {
    const safe = path.basename(String(fileName || ''));
    if (!safe || !safe.startsWith('backup-') || !safe.endsWith('.json.gz')) return null;
    const filePath = path.join(backupsDir(), safe);
    if (!fs.existsSync(filePath)) return null;
    try {
        const gz = fs.readFileSync(filePath);
        return { fileName: safe, filePath, bundle: JSON.parse(zlib.gunzipSync(gz).toString('utf8')) };
    } catch (e) {
        return null;
    }
}
...
function listBackups() {
    const dir = backupsDir();
    if (!fs.existsSync(dir)) return [];
    const files = fs.readdirSync(dir).filter(f => f.startsWith('backup-') && f.endsWith('.json.gz'));
    const out = [];
    for (const fileName of files) {
        const read = readBackupBundle(fileName);
        if (!read) continue;
        const { size } = fs.statSync(read.filePath);
        const campaigns = read.bundle.campaigns || {};
        const campaignCount = Object.keys(campaigns).length;
        const characterCount = Object.values(campaigns)
            .reduce((sum, c) => sum + Object.keys(c.characters || {}).length, 0);
        out.push({
            fileName,
            createdAt: read.bundle.createdAt,
            reason: read.bundle.reason,
            size,
            campaignCount,
            characterCount
        });
...
/** Restore one backup. Always takes a forced safety snapshot of current state first. */
function restoreBackup(fileName) {
    const read = readBackupBundle(fileName);
    if (!read) return { status: 404, error: 'Backup not found' };

    createBackupSnapshot({ force: true, reason: 'pre-restore' });

    const { bundle } = read;
    atomicWriteJson(manifestPath(), bundle.manifest);

    for (const [cid, camp] of Object.entries(bundle.campaigns || {})) {
        ensureDir(path.join(campaignDir(cid), 'characters'));
        bumpBeyondCurrentAndWrite(metaPath(cid), camp.meta);
        bumpBeyondCurrentAndWrite(mapPath(cid), camp.map);
        bumpBeyondCurrentAndWrite(combatPath(cid), camp.combat);
        for (const [charId, doc] of Object.entries(camp.characters || {})) {
            bumpBeyondCurrentAndWrite(characterPath(cid, charId), doc);
        }
    }

    return { status: 200, restoredFrom: fileName, restoredAt: nowIso() };
}

/** Write a restored doc with its revision bumped strictly above whatever's currently live,
 *  so every connected client hits the app's existing conflict/reload path on next poll. */
function bumpBeyondCurrentAndWrite(filePath, doc) {
    if (!doc) return;
    const current = readDoc(filePath);
    const currentRev = (current && current.revision) || 0;
    const restoredRev = doc.revision || 0;
    const next = { ...doc, revision: Math.max(currentRev, restoredRev) + 1, updatedAt: nowIso() };
...
function claimSeat({ campaignId, characterId, label, steal, passphrase }) {
    ...
    const prevClaim = doc.claim && doc.claim.sessionId ? doc.claim : null;
    if (prevClaim && prevClaim.sessionId) {
        const last = Date.parse(prevClaim.lastSeen || 0);
        const alive = last >= Date.now() - CLAIM_TTL_MS;
        // Alive claim held by a still-known session (or just recently seen) blocks unless steal.
        if (alive && !steal) {
            return {
                status: 409,
                ...
                reason: 'seat_claimed',
                claim: {
                    label: prevClaim.label || 'Player',
                    claimedAt: prevClaim.claimedAt,
                    lastSeen: prevClaim.lastSeen
                    // sessionId omitted from client response
                },
                canSteal: true
            };
        }
    ...
    const previousSessionId = prevClaim && prevClaim.sessionId ? prevClaim.sessionId : null;
    ...
    // Ethan #3: mark previous holder revoked so they get seat_taken on heartbeat/write.
    if (previousSessionId && previousSessionId !== sess.token) {
        markSessionSeatTaken(previousSessionId);
    }
    ...
    return {
        ...
        campaignId,
        label: sess.label,
        tookOver: !!(previousSessionId && previousSessionId !== sess.token)
    };
}
...
function heartbeat(token) {
    const raw = getSessionRaw(token);
    if (!raw) {
        return { status: 401, error: 'Invalid session', reason: 'invalid_session' };
    }
    if (raw.revoked === 'seat_taken') {
        revokeSessionOnly(token);
        return {
            status: 409,
            error: 'Your seat was taken by another device.',
            reason: 'seat_taken'
        };
    }
    if (raw.revoked) {
        ...
        return {
            status: 409,
            error: 'Your session was revoked.',
            reason: raw.revoked
        };
    }
    ...
    if (sess.role === 'player' && sess.characterId && sess.campaignId) {
        ...
        const doc = readDoc(p);
        if (!doc) {
            markSessionSeatTaken(token);
            revokeSessionOnly(token);
            return {
                status: 409,
                error: 'Your seat is gone (character missing).',
                reason: 'seat_gone'
            };
        }
        if (doc.claim && doc.claim.sessionId && doc.claim.sessionId !== token) {
            // Another device holds this seat — notify then drop.
            revokeSessionOnly(token);
            return {
                status: 409,
                error: 'Your seat was taken by another device.',
                reason: 'seat_taken',
                claim: {
                    label: doc.claim.label || 'Player',
                    lastSeen: doc.claim.lastSeen
                }
            };
        }
        if (doc.claim && doc.claim.sessionId === token) {
            doc.claim.lastSeen = nowIso();
            writeDoc(p, doc);
        }
        // No claim / expired claim: keep session alive; next write may reclaim via putCharacter.
    }
    return {
        status: 200,
        ok: true,
        session: { role: sess.role, characterId: sess.characterId, campaignId: sess.campaignId }
    };
}
...
function putCharacter(campaignId, charId, { baseRevision, data, sessionToken }) {
    ...
    if (isDm) {
        ...
        doc.revision = (doc.revision || 0) + 1;
        doc.updatedAt = nowIso();
        stampLastSavedBy(doc, (sess && sess.label) || 'DM', 'dm');
        writeDoc(p, doc);
    ...
    if (isHolder) {
        ...
        stampLastSavedBy(doc, (sess && sess.label) || charId, 'player');
        writeDoc(p, doc);
        return { status: 200, revision: doc.revision, mode: 'direct' };
    }

    // Seat was taken over: do not let a stale "own seat" session reclaim or write.
    if (isOwnSeat && doc.claim && doc.claim.sessionId && doc.claim.sessionId !== sessionToken) {
        return {
            status: 409,
            error: 'Your seat was taken by another device.',
            reason: 'seat_taken',
    ...
    // Claimed by someone else without a valid player session — do not accept anonymous writes.
    if (doc.claim && doc.claim.sessionId) {
        const raw = getSessionRaw(sessionToken);
        if (raw && raw.revoked === 'seat_taken') {
            return {
                status: 409,
                error: 'Your seat was taken by another device.',
                reason: 'seat_taken',
        ...
        return {
            status: 401,
            error: 'Valid seat session required to edit a claimed character.',
            reason: 'invalid_session'
        };
    }

    // Unclaimed + no session: legacy direct write
    doc.data = mergePreservePrivateFields(doc.data, data, false);
    doc.revision = (doc.revision || 0) + 1;
    doc.updatedAt = nowIso();
    stampLastSavedBy(doc, 'Unknown', null);
    writeDoc(p, doc);
...
function acceptOffer(campaignId, charId, offerId, sessionToken) {
    ...
    doc.revision = (doc.revision || 0) + 1;
    doc.updatedAt = nowIso();
    // Attribute to the offer's original author (the actual source of the change),
    // not whoever clicked accept.
    stampLastSavedBy(doc, offer.fromLabel || 'Helper', 'player');
    writeDoc(p, doc);
...
function putPiece(filePath, baseRevision, data, sessionToken) {
    const doc = readDoc(filePath);
    ...
    doc.revision = (doc.revision || 0) + 1;
    doc.updatedAt = nowIso();
    stampLastSavedBy(doc, (sess && sess.label) || 'Unknown', (sess && sess.role) || null);
    writeDoc(filePath, doc);
    return { status: 200, revision: doc.revision };
}

function putCombat(campaignId, baseRevision, data, sessionToken) {
    return putPiece(combatPath(campaignId), baseRevision, data, sessionToken);
}

function putMap(campaignId, baseRevision, data, sessionToken) {
    return putPiece(mapPath(campaignId), baseRevision, data, sessionToken);
}

function putMeta(campaignId, baseRevision, data, sessionToken) {
    return putPiece(metaPath(campaignId), baseRevision, data, sessionToken);
}
