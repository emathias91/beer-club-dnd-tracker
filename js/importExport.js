// JSON import/export, reset database (F4 safer), and the admin-tools chrome
// (import/export/reset button visibility + DM notes nav) that ties them
// together with every other subsystem's DM-only chrome.
import { renderAll } from '../app.js';
import { state, getActiveCampaign, saveLocalUi } from './state.js';
import { canUseDestructiveAdmin } from './auth.js';
import {
    IS_SERVER_MODE, setSyncStatus, sessionHeaders, saveStateToServer, logRoll
} from './sync.js';
import { escapeHtml, downloadJsonBlob } from './utils.js';
import { renderCampaignSelector, updateEmptyCampaignChrome } from './campaigns.js';
import { updateCharSheetChrome } from './characters.js';
import { updateMapUploadChrome } from './map.js';
import { updateCombatAndSessionChrome } from './combat.js';

/** Shared campaign document only (no seat chrome, revisions, locks, local UI). */
export function buildSharedExportPayload() {
    return {
...
export function updateAdminToolsChrome() {
...
export async function refreshExportStaleBanner() {
...
export function summarizeImportPayload(payload, meta) {
...
export async function applyFullStateReplace(payload, logLabel) {
